...
describe('attendance helper', () => {
  ...
  it('등록 시간 기준 07:11은 지각으로 판정한다', () => {
    const status = classifyAttendanceStatus('0700', new Date('2026-03-24T07:11:00'));
    expect(status).toBe('late');
    expect(getAttendanceStatusLabel(status)).toBe('지각');
  });

  it('13:00 직전인 12:59까지는 지각으로 판정한다', () => {
    const status = classifyAttendanceStatus('0700', new Date('2026-03-24T12:59:00'));
    expect(status).toBe('late');
    expect(getAttendanceStatusEmoji(status)).toBe('🟡');
  });

  it('13:00 정각이면 결석으로 판정한다', () => {
    const status = classifyAttendanceStatus('0700', new Date('2026-03-24T13:00:00'));
