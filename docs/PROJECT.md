...
| Workflow            | 트리거                                      | 역할                                                                                                                                                                                                               |
| ------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `CI`                | `push`, `pull_request`, `workflow_dispatch` | `ubuntu-22.04` + Node.js 24에서 lint, prettier, unit test, bot boot smoke test를 실행하고, 같은 저장소 PR/main push/manual 실행에서는 integration test 전에 테스트용 `config.json`과 slash command sync를 준비한다 |
...
- `CI`의 integration-test job은 같은 저장소 PR, `main` push, `workflow_dispatch`에서만 실행되고, 실행 전에 `scripts/write-integration-config.mjs`로 테스트용 `config.json`을 만든 뒤 `npm run deploy:commands`로 길드 slash command를 현재 코드 기준으로 전체 교체한다.
