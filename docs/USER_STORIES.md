...
- `#start-here`에는 서버 소개와 함께 설명 문구 없는 bot-owned persistent self-service 버튼 UI(`캠스터디 참여`, `기상챌린지 참여`)가 고정 안내로 제공된다
- 기상 self-service 안내 채널에는 설명 문구 없는 bot-owned persistent `기상 등록/수정`, `휴가 신청`, `기상 중단` 버튼 UI가 고정 안내로 제공된다
...
sequenceDiagram
    ...
    U->>S: 서버 소개와 UI-only persistent self-service 버튼 확인
    S-->>U: 버튼으로 참여 시작, 캠스터디 신청, 질문 채널 안내
    alt 질문이 있음
        U->>Q: 질문 남김
    end
```
...
```
AS A 서버 사용자
I WANT TO #start-here 의 상시 버튼 UI에서 캠스터디 참여를 바로 활성화하고 싶다
SO THAT 운영자 개입 없이 필요한 전용 채널 접근이 즉시 열리길 원한다
```

**인수 조건:**

- `#start-here`의 `캠스터디 참여` 버튼이 1차 진입점이고 `/캠스터디신청` (`/apply-cam`)은 fallback 으로 유지된다
- `캠스터디 참여` 버튼 또는 `/캠스터디신청` fallback 은 `#start-here`에서만 실행된다
- 다른 채널에서 `캠스터디 참여` 또는 `/캠스터디신청`을 실행하면 `#start-here` 사용과 `#qna` 질문 채널을 함께 안내한다
...
- 캠스터디는 이후 수동 역할 부여/회수와 서버 닉네임 변경도 `guildMemberUpdate`로 감지해서 `CamStudyUsers`를 계속 동기화한다
...
```
AS A 챌린저
I WANT TO 온보딩 버튼 UI 또는 /기상등록 fallback 으로 내 기상시간을 등록하거나 수정
...
- 기상시간을 입력받는다
- 기상시간은 `HHmm` 또는 `HH:mm` 형식으로 입력하고 05:00~09:00 범위만 허용한다
- `#start-here`의 `기상챌린지 참여` 버튼과 기상 self-service 안내 채널의 `기상 등록/수정` 버튼이 1차 진입점이고 `/기상등록`은 fallback 으로 유지된다
- `기상챌린지 참여` 버튼 또는 `기상 등록/수정` 버튼, `/기상등록` fallback 은 `#start-here`, 기상 self-service 안내 채널에서만 실행된다
- 응답은 실행한 사용자에게만 보이는 `ephemeral`로 반환된다
- 성공/실패 결과는 운영 확인용으로 `testChannelId`에도 기록된다
- Discord 사용자 ID는 interaction 사용자 정보에서 사용한다
- 저장/응답에 사용할 이름은 서버 display name을 우선 사용하고, 없으면 `globalName`, 그것도 없으면 `username`으로 fallback 한다
- 현재 시각 기준 `yearmonth`를 내부에서 계산한다
- `WakeUpMembership`이 없으면 생성하고, `stopped` 상태면 다시 `active` 로 전환한다
- 성공 시 `@wake-up` 역할이 함께 부여되고, 역할 부여 실패 시 DB 등록은 남지 않는다
- 기본 휴가일수는 5일
- 현재 월 `Users` 스냅샷이 없으면 자동 생성한다
- 이미 등록된 사용자는 정보가 업데이트된다
- 서버 닉네임을 바꾸면 이후 `guildMemberUpdate`로 활성 `WakeUpMembership`과 현재 월 `Users` 스냅샷 이름이 최신값으로 갱신된다
...
sequenceDiagram
    ...
    U->>D: 기상챌린지 참여 또는 기상 등록/수정 버튼 클릭
    D-->>U: modal "기상시간"
    U->>D: 07:00 입력 후 제출
    ...
    B->>B: waketime 정규화 및 유효성 검사 (HHmm/HH:mm -> HHmm, 0500~0900)
    alt 유효하지 않은 시간
        B-->>U: "기상시간은 05:00부터 09:00 사이 HHmm 또는 HH:mm 형식으로 입력해주세요"
    end

    B->>B: 이름 결정 (display name -> globalName -> username)
...
### US-1B: 서버 닉네임 변경 후 표시 이름 동기화

```
AS A 기상/캠스터디 참여자
I WANT TO 서버 닉네임을 바꾸면 이후 응답과 집계에 최신 이름이 반영되길 원한다
SO THAT 운영자와 다른 참가자가 결과표에서 현재 이름으로 나를 식별할 수 있다
```

**인수 조건:**

- `guildMemberUpdate`는 역할 변경이 없어도 서버 닉네임(display name) 변경만으로 동기화를 수행한다
- 활성 `WakeUpMembership.username`은 최신 이름으로 갱신된다
- 현재 월 `Users.username` 스냅샷이 있으면 최신 이름으로 갱신되고, 없으면 새 row를 만들지 않는다
- `@cam-study` 역할을 유지 중인 사용자의 `CamStudyUsers.username`은 최신 이름으로 갱신된다
- 진행 중 `CamStudyActiveSession`이 있으면 후속 종료 메시지와 정산에 쓰일 `username`도 최신 이름으로 갱신된다
- 이름 결정 우선순위는 `display name -> globalName -> username`으로 기상 self-service와 캠스터디 동기화에서 일관된다
...
sequenceDiagram
    ...
    B->>B: 이름 결정 (display name -> globalName -> username)
    B->>DB: WakeUpMembership.username 갱신 (active 인 경우)
    B->>DB: Users.username 갱신 (현재 월 row 가 있으면)
    B->>DB: CamStudyUsers.username 갱신 (@cam-study 유지 중이면)
    B->>DB: CamStudyActiveSession.username 갱신 (진행 중 세션이 있으면)
```
...
```
AS A 챌린저
I WANT TO 기상 중단 버튼 또는 /기상중단 fallback 으로 기상 챌린지 상시 참여를 직접 중단
...
- `기상 중단` 버튼이 1차 진입점이고 `/기상중단`은 fallback 으로 유지된다
- `기상 중단` 버튼 또는 `/기상중단` fallback 은 `#start-here`, 기상 self-service 안내 채널에서만 실행된다
- 버튼 경로는 destructive action 전에 확인 단계를 거친다
- 응답은 실행한 사용자에게만 보이는 `ephemeral`로 반환된다
- 성공/실패 결과는 운영 확인용으로 `testChannelId`에도 기록된다
...
- 성공 시 `WakeUpMembership.attendancestreak` 는 `0`, `attendancestreakupdatedon` 은 `null`로 초기화된다
...
- 등록 시간보다 10분 초과로 이른 댓글도 `attended`로 인정한다.
- 얼리 출석 댓글에는 `✅`와 함께 보조 반응 `🌅`를 추가하고, `⏰`는 공식 임시 반응으로 사용하지 않는다.
...
- 각 상태 그룹 내부에서는 기상시간이 빠른 사용자부터 먼저 표시한다
...
- 주말/공휴일 `attended`에는 등록 시간보다 10분 초과로 이른 댓글도 포함되며, `attended`/`late` 댓글에는 기존 판정 이모지와 함께 `🎁` 반응이 추가된다
- 주말/공휴일에는 무댓글, `absent`로 새 패널티를 추가하지 않는다
- 주말/공휴일 `attended`, `late`는 `absencecount`를 우선 1 감소시키고, 차감할 결석이 없으면 `latecount`를 1 감소시킨다
- 연속 출석 streak는 `WakeUpMembership`에 저장하며, 평일 `attended` 와 주말/공휴일 `attended` / `late` 에서만 1 증가한다
- 평일 `late` / `absent` / 무댓글은 streak 를 `0`으로 초기화하고, 평일 휴가와 주말/공휴일 무참여는 streak 를 유지한다
...
sequenceDiagram
    ...
    Note over T: ### 2025-12-08 출석표<br/>- 홍길동(06:00): 출석(연속 4회)<br/>- 김아침(07:00): 출석(연속 2회)<br/>- 이영희(07:30): 지각(누적 3회)<br/>- 박민수(08:00): 휴가(잔여 4일)<br/>- 최민지(06:30): 결석(누적 2회)
```
...
- 이름 저장 우선순위는 `display name -> globalName -> username`이다
...
- 기존 학습 기록은 유지된다
- 역할 유지 상태에서 서버 닉네임만 바뀌어도 `CamStudyUsers`와 진행 중 `CamStudyActiveSession` 이름은 최신값으로 갱신된다
...
```
AS A 챌린저
I WANT TO 휴가 신청 버튼 또는 /휴가신청 fallback 으로 자신의 휴가를 날짜 단위로 직접 등록
...
- 등록된 사용자만 가능
- 본인 데이터만 변경 가능
- `휴가 신청` 버튼이 1차 진입점이고 `/휴가신청`은 fallback 으로 유지된다
- `휴가 신청` 버튼 또는 `/휴가신청` fallback 은 `#start-here`, 기상 self-service 안내 채널에서만 실행된다
- 응답은 실행한 사용자에게만 보이는 `ephemeral`로 반환된다
- 성공/실패 결과는 운영 확인용으로 `testChannelId`에도 기록된다
