...
- 결과표에 사용자별 오늘 상태와 `latecount`, `absencecount`, 잔여휴가를 함께 표시
...
sequenceDiagram
    ...
    Note over T: ### 2025-12-08 출석표<br/>- 홍길동: 출석 (지각 0회, 결석 0회, 잔여휴가 5일)<br/>- 이영희: 지각 (지각 3회, 결석 1회, 잔여휴가 5일)<br/>- 박민수: 휴가 (지각 0회, 결석 0회, 잔여휴가 4일)<br/>- 최민지: 결석 (지각 0회, 결석 2회, 잔여휴가 5일)
```
