...
- `#apply`에는 참여 방법과 신청 명령어가 고정 안내로 제공되며 일반 대화는 허용하지 않는다
- `#apply` 고정 안내에는 "일반 메시지 금지 / 질문은 #qna / 명령어만 입력" 규칙이 포함된다
- `#qna`는 질문/응답 채널로 분리된다
- `#announcements`는 운영 공지 전용 채널로 분리된다
- `#start-here`, `#apply`, `#qna`, `#announcements`는 누구나 볼 수 있고 이후 전용 채널은 역할 기반으로만 노출된다
...
- 다른 채널에서 `/apply-wakeup`, `/apply-cam`을 실행하면 `#apply` 사용과 `#qna` 질문 채널을 함께 안내한다
...
- 이미 `@wake-up` 역할이 활성화된 사용자가 `/apply-wakeup`을 다시 실행하면 `/register` 다음 행동을 다시 안내한다
