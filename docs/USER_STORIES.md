...
- `#start-here`에는 참여 방법과 bot-owned persistent self-service 버튼 UI가 함께 안내된다
- 기상 self-service 안내 채널에는 bot-owned persistent `기상 등록/수정`, `기상 중단`, `휴가 신청` 버튼 UI가 고정 안내로 제공된다
...
sequenceDiagram
    ...
    U->>S: 서버 소개와 persistent self-service UI 확인
...
- `기상 등록/수정` 버튼이 1차 진입점이고 `/기상등록`은 fallback 으로 유지된다
- `기상 등록/수정` 버튼 또는 `/기상등록` fallback 은 `#start-here`, 기상 self-service 안내 채널에서만 실행된다
