...
- 역할 기반 운영 흐름을 추가할 때는 `#start-here` 같은 온보딩 채널과 `#ops` 같은 운영 채널을 분리하고, 신청 응답은 가능하면 `ephemeral`로 처리한다.
...
- `interactionCreate.ts`에 커맨드별 허용 채널 분기가 추가되면, 어떤 커맨드가 `#start-here`/`#ops` 같은 전용 채널에 묶이는지 문서에 남긴다.
