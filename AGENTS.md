...
- 이슈 단계에서 failing test를 만들 수 있는지 먼저 판단하고, 가능하면 이슈 본문에 그 시나리오를 적는다.
- failing test를 만들기 어렵다면 이슈 본문에 이유와 대체 검증 절차를 남긴다.
...
- `회귀 테스트 계획`에는 먼저 실패해야 하는 테스트 시나리오를 우선 적는다.
- 실패 테스트가 가능하면 구현은 그 테스트를 먼저 추가하고 실제 red를 확인하는 것부터 시작한다.
- 실패 테스트가 어렵다면 이슈 본문에 이유와 대체 검증 절차를 적고, 구현 시작 전 이를 명시적으로 확인한다.
- 구현 계획은 2계층으로 나눈다.
