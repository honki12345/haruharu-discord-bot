...
- 평일 출석 판정은 등록 시간 `+11분~12:59`까지를 `late`, 13:00 집계 시점 무댓글 또는 13:00 이후 댓글을 `absent` 기준으로 유지한다.
...
- thread guide는 얼리 출석 `✅ + 🌅`, 평일 `+11분~12:59 지각`, `13:00 집계 무댓글 결석`, 주말/공휴일 `attended`, `late` 보너스 차감과 `🎁` 반응 규칙을 실제 동작과 일치하게 유지한다.
