...
- workflow는 역할을 분리한다.
  - `ci.yml`: lint / prettier / unit test / smoke test / integration test. integration test는 같은 저장소 PR, `main` push, `workflow_dispatch`에서만 실행하고, 실행 전 테스트용 `config.json` 생성 + slash command sync를 선행한다.
